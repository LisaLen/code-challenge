...
class Node:
    ...
    def is_valid(self, lt = None, gt = None):
        """Is this tree a valid BST?"""

        def _check(node, lt=None, gt=None):

            if not node:
                return True
            if lt and node.data > lt:
                return False
            if gt and node.data < gt:
                return False
            if not _check(node.right, lt, node.data):
                return False
            if not _check(node.left, node.data, gt):
                return False
            return True

        return _check(self, None, None)
