...
def maxpath(nodes):
    """Given list of nodes in triangle, return value of high-scoring path."""
...
def maxpath2(nodes):
    ...
    #contracting path - from max_node to root
    path = [max_node.value]
    current = max_node
    while current.parents:
        local_max = 0
        for parent in current.parents:
            if parent.value > local_max:
                local_max = parent.value
                local_max_node = parent
        path.append(local_max_node)
        current = local_max_node

    #contracting path - from max_node to the end
    ...
    while current.children:
        ...
        for child in current.children:
            if child.value > local_max:
                local_max = child.value
                local_max_node = child
        path.append(local_max_node)
        current = local_max_node
